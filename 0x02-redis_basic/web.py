...
def get_page(url: str) -> str:
    ...
    print(f"Incremented count for {url}. Current count: {redis_client.get(count_key).decode('utf-8')}")
...
#    test_url_slow = "http://slowwly.robertomurray.co.uk/delay/5000/url/http://www.google.com"
#    test_url_fast = "http://example.com"

#    print("--- Testing slow URL (first fetch) ---")
#    print(f"Content length: {len(get_page(test_url_slow))}") # First fetch, slow
#    print("--- Testing slow URL (cached fetch) ---")
#    print(f"Content length: {len(get_page(test_url_slow))}") # Second fetch, should be fast (cache hit)
#    import time
#    time.sleep(11) # Wait for cache to expire
#    print("--- Testing slow URL (after expiration) ---")
#    print(f"Content length: {len(get_page(test_url_slow))}") # Third fetch, slow again (cache miss)
